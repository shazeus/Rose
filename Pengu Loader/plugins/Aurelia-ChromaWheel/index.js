...
(function createFakeChromaButton() {
  ...
  const SPECIAL_BASE_SKIN_IDS = new Set([99007]); // 82054, 145070, 103085, 25080 removed - handled by AURELIA-FormsWheel
  const SPECIAL_CHROMA_SKIN_IDS = new Set([100001, 88888]); // 145071, 103086, 103087 removed - handled by AURELIA-FormsWheel
  // HOL skins handled by AURELIA-FormsWheel (should not show ChromaWheel buttons)
  ...
  // Shared bridge API (provided by AURELIA-Bridge plugin)
  let bridge = null;
  ...
  function handleChromaStateUpdate(data) {
    ...
    // HOL chromas (Kai'Sa and Ahri) are now handled by AURELIA-FormsWheel
    ...
    // Note: Mordekaiser handling removed - now handled by AURELIA-FormsWheel plugin
    const getButtonIconPathForMordekaiser = (chromaId) => {
      // This function is kept for compatibility but should not be used
      // Mordekaiser is now handled by AURELIA-FormsWheel
      return null;
    };

    // Helper to get buttonIconPath for Spirit Blossom Morgana forms
    // Note: Morgana handling removed - now handled by AURELIA-FormsWheel plugin
    const getButtonIconPathForMorgana = (chromaId) => {
      // This function is kept for compatibility but should not be used
      // Morgana is now handled by AURELIA-FormsWheel
      return null;
    };

    // Helper to get buttonIconPath for HOL chromas - removed, handled by AURELIA-FormsWheel
    ...
    if (data.selectedChromaId && data.chromaColor) {
      ...
      if (
        ...
        // Note: Mordekaiser (82054) and Spirit Blossom Morgana (25080) handling removed - now handled by AURELIA-FormsWheel plugin
        // HOL chromas (Kai'Sa and Ahri) are now handled by AURELIA-FormsWheel - skip here
      } else {
      ...
      // Note: Mordekaiser (82054), Spirit Blossom Morgana (25080), and HOL chromas (Kai'Sa and Ahri) are now handled by AURELIA-FormsWheel - skip here
  ...
  // HOL chroma functions removed - now handled by AURELIA-FormsWheel
  ...
  // getHolButtonIconPath removed - now handled by AURELIA-FormsWheel
  ...
  function ensureFakeButton(skinItem) {
    ...
    // Skip HOL skins - they are handled by AURELIA-FormsWheel
    ...
    const hasChromas = Boolean(
      ...
      // Note: Mordekaiser (82054), Spirit Blossom Morgana (25080), and HOL skins removed - handled by AURELIA-FormsWheel
    );
  ...
  function getChromaData(skinData) {
    ...
    // NOTE: Sahn Uzal Mordekaiser (82054) and Spirit Blossom Morgana (25080) removed - now handled by AURELIA-FormsWheel plugin
    // Previously these special cases handled Mordekaiser and Morgana forms, but they're now excluded from this plugin

    // SPECIAL CASE: Risen Legend Kai'Sa and Ahri HoL skins are now handled by AURELIA-FormsWheel
    // (removed - handled by AURELIA-FormsWheel)
  ...
  function createChromaPanel(skinData, chromas, buttonElement) {
    ...
    headerKicker.textContent = "aurelia aurelia";
  ...
  function updateChromaPreview(chroma, chromaImage) {
    ...
    // Note: Mordekaiser removed - handled by AURELIA-FormsWheel
  ...
  function updateChromaButtonColor() {
    ...
    buttons.forEach((button) => {
      ...
      // Note: Mordekaiser (82054), Spirit Blossom Morgana (25080), and HOL chromas are now handled by AURELIA-FormsWheel
      const isElementalistLux =
        selectedChromaData &&
        (selectedChromaData.id === 99007 ||
          (selectedChromaData.id >= 99991 && selectedChromaData.id <= 99999));
      // HOL chromas are now handled by AURELIA-FormsWheel
      const isDefault =
        !selectedChromaData ||
  ...
  function toggleChromaPanel(buttonElement, skinItem) {
    ...
    // Don't open panel for HOL skins - they are handled by AURELIA-FormsWheel
    const currentSkinId = skinMonitorState?.skinId ?? null;
    if (currentSkinId && HOL_SKIN_IDS.has(currentSkinId)) {
