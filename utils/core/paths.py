...
def _get_desktop_user_info() -> Tuple[Optional[str], Optional[str]]:
    ...
    even if Aurelia is running elevated as a different admin account.
...
def get_user_data_dir() -> Path:
    ...
    admin account. This ensures that Aurelia's data directory matches what Pengu
    Loader (running in League's process) will use.

    This handles the case where:
    - User "daish" is logged into Windows
    - User runs Aurelia as "Admin" (different admin account)
    - Aurelia needs to use daish's AppData, not Admin's AppData
    """
    ...
    if os.name == "nt":  # Windows
        ...
        # This handles the case where Aurelia runs as a different admin account
        ...
        if desktop_username and desktop_profile:
            if desktop_username.lower() != current_username:
                ...
                if desktop_localappdata:
                    _cached_user_data_dir = desktop_localappdata / "Aurelia"
        ...
        if localappdata:
            _cached_user_data_dir = Path(localappdata) / "Aurelia"
            return _cached_user_data_dir

        # Fallback to user profile
        userprofile = os.environ.get("USERPROFILE")
        if userprofile:
            _cached_user_data_dir = Path(userprofile) / "AppData" / "Local" / "Aurelia"
            return _cached_user_data_dir
        ...
        if xdg_data_home:
            _cached_user_data_dir = Path(xdg_data_home) / "Aurelia"
        else:
            _cached_user_data_dir = Path.home() / ".local" / "share" / "Aurelia"
        return _cached_user_data_dir


def get_appdata_dir() -> Path:
    """
    Get the Aurelia AppData directory.
