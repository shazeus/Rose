...
def test_release_api_defaults_to_shazeus_fork(monkeypatch):
    monkeypatch.delenv("ROSE_RELEASE_API", raising=False)
    monkeypatch.delenv("ROSE_RELEASE_REPO", raising=False)

    assert github_client.GITHUB_RELEASE_API == "https://api.github.com/repos/shazeus/Rose/releases/latest"
    assert github_client.get_release_api_url() == "https://api.github.com/repos/shazeus/Rose/releases/latest"


def test_release_api_accepts_repo_override(monkeypatch):
    monkeypatch.delenv("ROSE_RELEASE_API", raising=False)
    monkeypatch.setenv("ROSE_RELEASE_REPO", "example/Fork")
...
def test_release_api_accepts_full_url_override(monkeypatch):
    monkeypatch.setenv("ROSE_RELEASE_API", "https://example.test/releases/latest")
    monkeypatch.setenv("ROSE_RELEASE_REPO", "example/Fork")
